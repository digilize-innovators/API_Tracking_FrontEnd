   ...
   useEffect(() => {
    ...
    const handleGenerateReport = async () => {
        if (!selectedProduct || !selectedBatch || !selectedFormat) {
            setAlertData({ type: 'warning', message: 'Please select both product,batch,Report Format before generating report.', variant: 'filled' });
            setOpenSnackbar(true);
    ...
    const AggregateXml = async() => {
        ...
        let AggregationFile
          ...
          if(!AggregationFile)
          {
            setAlertData({ type: 'warning', message: 'file Name not found', variant: 'filled' });
            setOpenSnackbar(true);
        ...
        const response = await api('/batch/reportFormat', {
            fileName:AggregationFile,
    ...
    const  batchRecordPdf =async(approver)=>{
        ...
        const headerContent = () => {
            ...
            doc.setFontSize(16).setFont(undefined, 'bold').text('Fourrts (India) Laboratories Pvt. Ltd.', 90, 16,{ align: 'center' });
        ...
        const footerContent = (pageNumber, totalPages) => {
            ...
            doc.text(
                `Date: ${moment(exportedBy.exportedAt || new Date()).format(
        ...
        const renderBatchDetails = () => {
            ...
            const rows =
                [
                    ...
                    BatchDetail.manufacturing_date
                        ? moment(BatchDetail.manufacturing_date).format('DD/MM/YYYY ')
                        : 'N/A',
                    BatchDetail.expiry_date
                        ? moment(BatchDetail.expiry_date).format('DD/MM/YYYY ')
                        : 'N/A',
                    BatchDetail.created_at
                        ? moment(BatchDetail.created_at).format('DD/MM/YYYY ')
                        : 'N/A',
        ...
        doc.setFontSize(12).setFont(undefined, 'bold');
        currentY += 10;
        (name=='primary')?  doc.text('Aggregation Report Primary to Secondary Codes:', 14, currentY):  
        ...
        autoTable(doc, {
            ...
            body: code.map((item,key)=>[
                key+1,item.t1_unique_code,item.t2_unique_code
            ]),
