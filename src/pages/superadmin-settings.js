...
const Index = () => {
  ...
  const getData = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setGenerateCode({
          ...generateCode,
          type: config.codes_type ?? 'random',
          length: config.code_length ?? '',
          productCodeLength: config.product_code_length ?? '',
          errorMessage: '',
          isError: false,
          errorMessageProduct: '',
          isErrorProduct: false
  ...
  const handleCodeLength = event => {
    ...
    if (intVal >= 6 && intVal <= 8) {
      ...
      setGenerateCode({
        ...generateCode,
        length: intVal,
        errorMessage: 'Code length must be between 6 and 8',
        isError: true
      })
    }
  }
  ...
  const handleProductCodeLength = event => {
    ...
    if (intVal >= 2 && intVal <= 4) {
      ...
      setGenerateCode({
        ...generateCode,
        productCodeLength: intVal,
        errorMessageProduct: 'Product Code length must be between 2 and 4',
        isErrorProduct: true
      })
    }
  }
  ...
  return (
    <Box>
      ...
      <Dialog open={openGenerateDialog} onClose={handleCloseGenerateDialog} aria-labelledby='generate-dialog'>
        <DialogContent>
          <Grid2 container spacing={2}>
      ...
      <Dialog open={openConfirm} onClose={handleCloseConfirm} aria-labelledby='confirm-dialog'>
        <Typography id='confirm-dialog' variant='h4' sx={{ mx: 4, mt: 8 }}>
