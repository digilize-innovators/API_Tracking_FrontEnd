...
import TableMaterialIssue from 'src/views/tables/TableMaterialIssue'
...
import MaterilIssueModel from 'src/components/Modal/MaterialIssueModal'
...
const Index = () => {
  ...
  const apiAccess = useApiAccess('materialissue-create', 'materialissue-update', 'materialissue-approve')
  const searchBarRef = useRef(null)
  const [materialIssueData, setmaterialIssueData] = useState({ data: [], index: 0 })
    ...
    const handleUserAction = async () => {
      if (formData && pendingAction) {
        ...
        if (pendingAction === 'edit') {
          await editMaterialIssue(esign_status)
        } else if (pendingAction === 'add') {
          await addMaterialIssue(esign_status)
        }
  ...
  const handleOpenModal = () => {
    setApproveAPI({ approveAPIName: 'materialissue-create', approveAPImethod: 'POST', approveAPIEndPoint: '/api/v1/materialissue' })
    setEditData({})
    setOpenModal(true)
  }
  ...
  const handleSubmitForm = async data => {
    ...
    if (editData?.id) {
      setApproveAPI({ approveAPIName: 'materialissue-update', approveAPImethod: 'PUT', approveAPIEndPoint: '/api/v1/materialissue' })
    } else {
      setApproveAPI({ approveAPIName: 'materialissue-create', approveAPImethod: 'POST', approveAPIEndPoint: '/api/v1/materialissue' })
    }
  ...
  const addMaterialIssue = async esign_status => {
    try {
      const data = { ...formData }
      ...
      setIsLoading(true)
      const res = await api('/materialissue/', data, 'post', true)
      setIsLoading(false)
      if (res?.data?.success) {
        setAlertData({ ...alertData, openSnackbar: true, type: 'success', message: 'Material Issue added successfully' })
        resetForm()
        setOpenModal(false)
      } else {
      ...
      console.log('Error to add Material Issue', error)
      router.push('/500')
    } finally {
  ...
  const editMaterialIssue = async esign_status => {
    try {
      ...
      data.esign_status = esign_status
      setIsLoading(true)
      const res = await api(`/materialissue/${editData.id}`, data, 'put', true)
      setIsLoading(false)
      if (res.data.success) {
        setAlertData({ ...alertData, openSnackbar: true, type: 'success', message: 'Material Issue updated successfully' })
        resetForm()
        setOpenModal(false)
      } else {
        setAlertData({ ...alertData, openSnackbar: true, type: 'error', message: res.data.message })
...
const handlePdfExport = async (remarks, user) => {
  downloadPdf(tableData, tableHeaderData, tableBody, materialIssueData.data, user);
  
  if (config?.config?.audit_logs) {
    const auditData = {
      audit_log: {
        audit_log: true,
        performed_action: 'Export report of Material Issue',
        remarks: remarks?.length > 0 ? remarks : 'Material Issue export report',
        authUser: user
      }
    };
...
const updateEsignStatus = async (esignStatus, remarks, user) => {
  const data = {
    ...
    audit_log: config?.config?.audit_logs ? {
      ...
      remarks: remarks?.length > 0 ? remarks : `materialissue ${esignStatus} - ${auditLogMark}`,
...
const handleCreatorApproval = (remarks, user,isApprover) => {
  ...
  } else if (!isApprover && approveAPI.approveAPIName === 'materialissue-approve') {
    setAlertData({
      ...alertData,
      openSnackbar: true,
      type: 'error',
  ...
  const handleAuthCheck = async row => {
    setApproveAPI({ approveAPIName: 'materialissue-approve', approveAPImethod: 'PATCH', approveAPIEndPoint: '/api/v1/materialissue' })
  ...
  const handleDownloadPdf = () => {
    ...
    downloadPdf(tableData, tableHeaderData, tableBody, materialIssue.data, userDataPdf)
  }
  ...
  return (
    <Box padding={4}>
      ...
      <Grid2 item xs={12}>
        <Typography variant='h2'>Material Issue</Typography>
      </Grid2>
      <Grid2 item xs={12}>
        <Grid2 item xs={12}>
            ...
            <Grid2 item xs={12}>
              <Typography variant='h4' className='mx-4 my-2 mt-3'>
                ...
                <TableMaterialIssue
                  ...
                  setDataCallback={setmaterialIssueData}
...
export async function getServerSideProps(context) {
  return validateToken(context, 'Material Issue')
}
