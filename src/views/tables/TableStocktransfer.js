  ...
  return (
    <Fragment>
      <TableRow sx={{ '& > *': { borderBottom: '1px solid rgba(224, 224, 224, 1)' } }}>
        ...
        <TableCell sx={{ borderBottom: '1px solid rgba(224, 224, 224, 1)' }} align='center' className='p-2'>
          {row.stofl.location_name}
        </TableCell>
        <TableCell sx={{ borderBottom: '1px solid rgba(224, 224, 224, 1)' }} align='center' className='p-2'>
          {row.stotl.location_name}
        </TableCell>
                    ...
                    <TableBody>
                      {historyData[row.id]?.map((historyRow, idx) => (
                        <TableRow key={historyRow.created_at} align='center' sx={{ borderBottom: '1px solid rgba(224, 224, 224, 1)' }}>
                          ...
                          <TableCell align='center' sx={{ borderBottom: '1px solid rgba(224, 224, 224, 1)' }} >{historyRow.stohfl.location_name}</TableCell>
                          <TableCell align='center' sx={{ borderBottom: '1px solid rgba(224, 224, 224, 1)' }} >{historyRow.stohtl.location_name}</TableCell>
                          {config?.config?.esign_status === true && config?.role!=='admin' && (
                            <StatusChip
...
const TableStocktransfer = ({
  ...
  const handleRowToggle = async (rowId) => {
    await handleRowToggleHelper(rowId, openRows, setOpenRows, setHistoryData, '/stocktransfer-order/history');
  };
  ...
  const getData = async () => {
    ...
    try {
      ...
      if (response?.data?.success) {
        setStocktransferData({data: response.data.data.stockTransferOrders, total:response.data.data.total})
        setStocktransfer(response.data.data.stockTransferOrders)
      } else {
