...
//   const isOpen = openRows[row.id];
 ...
 const handleProductDetail=(row)=>{
  ...
  return (
    <Fragment>
      <TableRow sx={{ '& > *': { borderBottom: '1px solid rgba(224, 224, 224, 1)' } }}>
        ...
        <TableCell align='center' sx={{ borderBottom: '1px solid rgba(224, 224, 224, 1)' }}>{row.product}</TableCell>
  ...
  <IconButton data-testid={`auth-check-icon-${row.id}`}
                onClick={() => handleProductDetail(row)}><PlayArrowIcon /></IconButton>
</Tooltip>
            </span>
            <span>
            ...
            <Tooltip title="Pause">
<IconButton data-testid={`auth-check-icon-${row.id}`}
              onClick={() => handleProductClose(row)}><StopIcon /></IconButton>
</Tooltip>
          </span>
          <span>
