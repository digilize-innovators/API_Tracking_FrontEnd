...
const locationSchema = yup.object().shape({
  ...
  locationName: yup
    ...
    .test('valid-location-name', 'Location Name can only contain letters, numbers, and single spaces', value => {
  ...
  mfgName: yup.string().trim().max(255, 'Mfg name length should be less than 255'),

  address: yup.string().trim().max(150, 'Address length should be less than 151')
})
      ...
      <Box sx={style}>
        ...
        <form onSubmit={handleSubmit(handleSubmitForm)}>
          <Grid2 container spacing={2}>
            ...
            <Grid2 size={6}>
              <CustomDropdown name='locationType' label='Location Type *' control={control} options={locationData} />
            </Grid2>
          </Grid2>

          <Grid2 container spacing={2}>
            <Grid2 size={6}>
              <CustomTextField name='locationName' label='Location Name *' control={control} />
            </Grid2>
            <Grid2 size={6}>
              <CustomTextField name='mfgLicenceNo' label='Mfg Licence No. *' control={control} />
            </Grid2>
          </Grid2>
