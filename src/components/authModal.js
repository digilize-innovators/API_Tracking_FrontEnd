...
import { Modal, Box, TextField, Button, Typography, Grid2, TextareaAutosize, InputAdornment, IconButton, Popover } from '@mui/material';
...
const AuthModal = ({ open, handleClose, handleAuthResult, approveAPIName, approveAPImethod, approveAPIEndPoint, config, handleAuthModalOpen, openModalApprove }) => {
    ...
    const fetchDesignations = async () => {
        ...
        try {
            const res = await api(`/designation/designationbyapis/${encodeURIComponent(params.method)}?endpoint=${encodeURIComponent(params.endpoint)}`, {}, 'get', true);
            if (res.data.success) {
                        ...
                        endAdornment: (
                            <InputAdornment position="end">
                                <IconButton onClick={togglePasswordVisibility} edge="end">
                                    {showPassword ? <FaEye color={settings.themeColor} /> : <AiFillEyeInvisible color={settings.themeColor} />}
                                </IconButton>
                            </InputAdornment>
                        ),
                    }}
